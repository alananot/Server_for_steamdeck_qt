...
#define TURRET_VERT_MAX 230                                     //The vertical servo maximum upwards angle//
...
//#define LIGHT_ENA 38
#define REARLIGHT2 39
#define LIGHT_IN1 40 // Baklyktor
#define LIGHT_IN2 38 // Framlyktor
#define MAX_VARIABLES 100
#define THREE_VOLT 4
#define FIVE_VOLT 5
#define FOURTEEN_VOLT 6
#define GAS_SHOT_PIN 37
#define RXD_PIN 44
#define TXD_PIN 43

#define THREE_VOLT_CURRENT 2
#define FIVE_VOLT_CURRENT 7
#define FOURTEEN_VOLT_CURRENT 1
...
void autonomousy(float x, float y, float dist){                             //Allows the top to follow autonomously//
 ...
 //       if()
  ...
  //          move(0.5f, 0);
    ...
    if((y < 400 +10  || y > 400 + 14 || x > 400 -16 || x < 400 -20) && !aimLocked){
        ...
        if(x < 400 -20)
        {
            ...
            {
                Serial.println("Left");
                h = -((x - 382)/382);                          //Changes the speed of autonomous turning based on distance from center//
                //h*=-1;
                /*if(x < 320-50)
                {
                    h = 0.5;
...
public:
    ...
    TurretMotor(int hor, int vert) {
        ...
        currentHorPos = TURRET_HOR_MID;
        currentVertPos = SERVO_MID;
        pwm.setPWM(TURRET_HOR, 0, TURRET_HOR_MID);
    ...
    void update() {
        ...
        if(abs(currentVertSpeed) > 1.0)
        {
            ...
            if (currentHorCam > CAM_MIN) currentHorCam = CAM_MIN;
            if (currentHorCam < CAM_MAX) currentHorCam = CAM_MAX;
            //Serial.println(currentVertPos);
            pwm.setPWM(vertChannel, 0, (int)currentVertPos);
                        pwm.setPWM(CAM_PIN, 0, (int)currentHorCam);
                        //Serial.println(currentHorCam);
        }
...
*/                        }
                    ...
                    if(currentHorCam > CAM_MAX +50)
                    {
                        currentHorCam -=1;
                    }
                    else if(currentHorCam < CAM_MAX +50)
                    {
                        currentHorCam += 1;
                    }
                    pwm.setPWM(TURRET_VERT,0,currentVertPos);
                    pwm.setPWM(CAM_PIN,0, currentHorCam);
                    if(!left){
                    currentHorPos +=1;
...
void handleGraph() {
    ...
    if (graphClient && graphClient.connected()) {
        ...
        if (millis() - lastGraphTime >= 50) {                   //Sends information about the voltage and current passing through the system to the steamdeck
            ...
            float Curr3v3 = analogReadMilliVolts(THREE_VOLT_CURRENT);
            float Curr14v = analogReadMilliVolts(FOURTEEN_VOLT_CURRENT);
            float Curr5v = analogReadMilliVolts(FIVE_VOLT_CURRENT);
            // 14.8 V
        ...
        if(totalAmount14v < MAX_VARIABLES){ // Makes sure to just use the 100 latest values
                totalPower14v[totalAmount14v] = Raw14v * 52.3;
                totalAmount14v ++;
            }
            else{
                ...
                totalPower14v[MAX_VARIABLES-1] = Raw14v * 52.3;
            }
            float sum14v = 0;
        ...
        if(totalAmount5v < MAX_VARIABLES){ // Makes sure to just use the 100 latest values
                totalPower5v[totalAmount5v] = Raw5v * 2;
                totalAmount5v ++;
            }
            else{
                for(int i = 0; i< MAX_VARIABLES -1; i++){
                ...
                totalPower5v[MAX_VARIABLES-1] = Raw5v * 2;
            }
            float sum5v = 0;
            ...
            graphClient.print(x2, 4);
            graphClient.print(","); 
            graphClient.print(y2, 4); 
            graphClient.print(",");
            graphClient.print(resistans, 4);
            graphClient.print(",");
            graphClient.print(power,4);
            graphClient.print(",");
            graphClient.print(check);
            graphClient.print(",");
            graphClient.print(Curr14v/1000,4);
            graphClient.print(",");
            graphClient.println(lidarDist,4);
            /*graphClient.print(averagePower3v/1000, 4);
            graphClient.print(","); 
            graphClient.print(averagePower5v/1000, 4); 
            graphClient.print(",");
            graphClient.print(averagePower14v/1000, 4);
            graphClient.print(",");
            graphClient.print(Curr3v3/1000,4);
            graphClient.print(",");
            graphClient.print(Curr5v/1000,4);
            graphClient.print(",");
            graphClient.print(Curr14v/1000,4);
            graphClient.print(",");
            graphClient.println(lidarDist,4);*/
        ...
        if (joystickClient.available()) {
            ...
            if (msg.length() > 0) {
                ...
                if (sscanf(msg.c_str(), "%d,%d,%d,%d", &v0, &v1, &v2, &v3) == 4) {
                                        ...
                                        if(((v0 != 0 && preV0 == 0) || (v1 != 0 && preV1 == 0)) && ((v2 == 0 && preV2 != 0) || (v3 == 0 && preV3 != 0)))
                    {
                        //Serial.println("Hello");
                        /*Serial.print("Car turn: ");
                        Serial.println(v0);
                        Serial.print("Car speed: ");
                        Serial.println(v1);
                        Serial.print("Turret Hor: ");
                        Serial.println(v2);
                        Serial.print("Turret Vert: ");
                        Serial.println(v3);*/
                        int saveV2 = v2;
                        int saveV3 = v3;
                        v2 = v0;
...
void setup() {
    ...
    pinMode(FOURTEEN_VOLT, OUTPUT);
        pinMode(FIVE_VOLT_CURRENT, OUTPUT);
    pinMode(THREE_VOLT_CURRENT, OUTPUT);
    pinMode(FOURTEEN_VOLT_CURRENT, OUTPUT);
...
void loop() {
   ...
   // Serial.println("YES");
    ...
    else if(!Serial1.available() && autonomous && currentTime -passedTime > 100){


    //    check = 9;

        searchCurrent = millis();
        if(searchCurrent - searchStart > 10)
        {
        idle_state();
