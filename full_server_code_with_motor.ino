...
void autonomousy(float x, float y, float dist){                             //Allows the top to follow autonomously//
 ...
 //       if()
  ...
  //          move(0.5f, 0);
    ...
    if((y < 400 +40  || y > 400 + 44 || x > 400 -40 || x < 400 -36) && !aimLocked){
        ...
        if(x < 400 -36)
        {
            ...
            {
                Serial.println("Left");
                h = -((x - 362)/362);                          //Changes the speed of autonomous turning based on distance from center//
                //h*=-1;
                /*if(x < 320-50)
                {
                    h = 0.5;
...
public:
    ...
    void update() {
        ...
        if(abs(currentVertSpeed) > 1.0)
        {
            ...
            if (currentHorCam > 340) currentHorCam = 340;
            if (currentHorCam < 250) currentHorCam = 250;
...
void handleGraph() {
    ...
    if (graphClient && graphClient.connected()) {
        ...
        if(totalAmount5v < MAX_VARIABLES){ // Makes sure to just use the 100 latest values
            ...
            graphClient.print(",");
            graphClient.print(current14v/1000,4);
            graphClient.print(",");
...
void loop() {
  ...
  // Serial.println(raw);
    ...
    /*else if(!Serial1.available() && autonomous){



        
        searchCurrent = millis();
        if(searchCurrent - searchStart > 1000)
        {
        idle_state();
        searchStart = millis();
        }
